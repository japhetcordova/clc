        ...
        <div className="min-h-screen bg-background text-foreground p-4 flex flex-col items-center justify-center relative overflow-hidden">
            ...
            <div className="w-full max-w-md space-y-6 relative z-10">
                <div className="text-center space-y-2">
                    ...
                    <p className="text-muted-foreground font-medium text-sm">Scan QR Code for entry verification</p>
                </div>

                <div className="relative aspect-square w-full bg-card rounded-[3rem] overflow-hidden ring-1 ring-border shadow-2xl">
                    <div id="reader" className="w-full h-full overflow-hidden [&_video]:object-cover"></div>

                    {!isScannerReady && (
                        <div className="absolute inset-0 bg-card/80 backdrop-blur-xl flex flex-col items-center justify-center p-8 text-center space-y-6">
                            <div className="w-20 h-20 bg-primary/10 rounded-full flex items-center justify-center ring-1 ring-primary/20">
                                <Camera className="w-10 h-10 text-primary" />
                            </div>
                            <div className="space-y-2">
                                <h3 className="text-xl font-black">{hasPermission === false ? "Access Blocked" : "Ready to Scan?"}</h3>
                                <p className="text-sm text-muted-foreground font-medium">
                                    {hasPermission === false ? "Enable camera in browser settings." : "Enable your camera to begin scanning."}
                                </p>
                            </div>
                            {hasPermission !== false && (
                                <Button onClick={requestPermissionAndStart} className="rounded-2xl h-12 px-8 font-bold bg-primary shadow-lg shadow-primary/20 transition-all hover:scale-105">
                                    Enable Camera
                                </Button>
                            )}
                        </div>
                    ...
                    {isScannerReady && (
                        <>
                            <div className="absolute inset-0 pointer-events-none border-[30px] border-background/20 opacity-50" />
                            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 border-2 border-primary/50 rounded-3xl pointer-events-none">
                            ...
                            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 z-30">
                                {cameras.length > 1 && (
                                    <Button size="icon" variant="secondary" className="h-12 w-12 rounded-full bg-background/80 backdrop-blur-md shadow-xl" onClick={toggleCamera}>
                                        <Repeat className="w-5 h-5" />
                                    </Button>
                                )}
                                <Button size="icon" variant="secondary" className="h-12 w-12 rounded-full bg-background/80 backdrop-blur-md shadow-xl" onClick={() => window.location.reload()}>
                    ...
                    <AnimatePresence>
                        {isProcessing && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-accent/20 backdrop-blur-2xl flex flex-col items-center justify-center z-40">
                                ...
                                <p className="font-black text-2xl mt-8 tracking-tighter text-foreground">Verifying...</p>
                            </motion.div>
                        )}

                        {lastScan && (
                            <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className={cn("absolute inset-x-6 bottom-6 p-6 rounded-[2rem] flex flex-col items-center text-center gap-3 border shadow-2xl z-50 backdrop-blur-2xl", lastScan.success ? "bg-emerald-500/20 border-emerald-500/50" : "bg-destructive/20 border-destructive/50")}>
                                {lastScan.success ? <CheckCircle2 className="w-10 h-10 text-emerald-500" /> : <XCircle className="w-10 h-10 text-destructive" />}
                                <div className="space-y-1">
                                    ...
                                    {lastScan.user && (
                                        <div className="flex items-center justify-center gap-2">
                                            <span className="px-2 py-0.5 bg-background/50 rounded-md text-[10px] font-black uppercase tracking-widest text-muted-foreground ring-1 ring-border">{lastScan.user.ministry}</span>
                                        </div>
                                    )}
                                </div>
                            </motion.div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <Card className="bg-card/40 border-border text-foreground backdrop-blur-xl ring-1 ring-border shadow-lg">
                        <CardContent className="p-4 flex flex-col items-center text-center space-y-2">
                            ...
                            <p className="font-bold text-xs">Live Sync</p>
                        </CardContent>
                    </Card>
                    <Card className="bg-card/40 border-border text-foreground backdrop-blur-xl ring-1 ring-border shadow-lg">
                        <CardContent className="p-4 flex flex-col items-center text-center space-y-2">
