...
import { User as UserIcon, Phone, Mail, Users as UsersIcon, Calendar, Share2, Download, Edit3, Settings, ShieldCheck, Lightbulb, Home, Eye, Sparkles, X, Play, Video } from "lucide-react";
...
export default function ProfileView({ user, qrValue, attendance = [], initialAuthorized }: ProfileViewProps) {
    ...
    const [selectedVideo, setSelectedVideo] = useState<{ className: string; module: string | null; video: string } | null>(null);
    const classVideosQuery = trpc.getClassVideos.useQuery();
                    ...
                    <div className="flex items-center justify-between w-full px-2 sm:px-0">
                        <div className="flex items-center bg-card/60 backdrop-blur-3xl rounded-[1.25rem] border border-border/50 p-1 shadow-2xl ring-1 ring-white/5 animate-in fade-in slide-in-from-top-4 duration-500">
                            <Link href="/mobile">
                                <Button
                                    variant="ghost"
                                    className="rounded-xl h-10 px-4 text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-all font-black uppercase text-[10px] tracking-widest gap-2"
                                >
                            ...
                            <Button
                                ...
                                className="rounded-xl h-10 px-4 text-destructive hover:bg-destructive/10 hover:text-destructive transition-all font-black uppercase text-[10px] tracking-widest gap-2"
                            >
                                <LogOut className="w-4 h-4" />
                                <span>Lock <span className="hidden xs:inline">Profile</span></span>
                            </Button>
                        </div>
                        ...
                        <div className="hidden sm:flex items-center gap-2 px-4 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
                            ...
                            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-emerald-600">Authenticated Session</span>
                        </div>
                    </div>
                        ...
                        <div className="space-y-6">
                            <Card className="h-full border-none shadow-xl bg-card/50 backdrop-blur-md ring-1 ring-border p-1">
                                <Tabs defaultValue="overview" className="h-full flex flex-col">
                                    <TabsList className="w-full justify-start p-1.5 sm:p-2 bg-transparent border-b border-border/50 rounded-none h-auto gap-1.5 sm:gap-2">
                                        {["overview", "activity", "classes"].map((tab) => (
                                            <TabsTrigger
                                                key={tab}
                                                value={tab}
                                                className="rounded-xl px-4 sm:px-6 py-2 sm:py-2.5 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground font-bold uppercase text-[10px] tracking-widest transition-all"
                                            >
                                                {tab}
                                            </TabsTrigger>
                                        ))}
                                    ...
                                    <div className="p-4 sm:p-6 md:p-8 flex-1">
                                        ...
                                        <TabsContent value="classes" className="mt-0 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                            <div className="space-y-6">
                                                ...
                                                {classVideosQuery.data && Object.keys(classVideosQuery.data).length > 0 ? (
                                                    <div className="grid gap-10">
                                                        {Object.entries(classVideosQuery.data).map(([className, videos]) => {
                                                            // Group videos by module for visual structure
                                                            const grouped = (videos as { module: string | null, file: string }[]).reduce((acc, v) => {
                                                                const m = v.module || "General";
                                                                if (!acc[m]) acc[m] = [];
                                                                acc[m].push(v);
                                                                return acc;
                                                            ...
                                                            return (
                                                                <div key={className} className="space-y-6">
                                                                    <div className="flex items-center gap-4">
                                                                        ...
                                                                        <h4 className="text-sm font-black uppercase text-foreground tracking-[0.2em]">{className} Class</h4>
                                                                            ...
                                                                            <div key={moduleName} className="space-y-4">
                                                                                {moduleName !== "General" && (
                                                                                    <div className="flex items-center gap-2">
                                                                                        ...
                                                                                        <h5 className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">{moduleName}</h5>
                                                                                    </div>
                                                                                )}
                                                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                                                    {moduleVideos.map((v) => (
                                                                                        <button
                                                                                            key={v.file}
                                                                                            onClick={() => setSelectedVideo({ className, module: v.module, video: v.file })}
                                                                                            className="flex items-center gap-4 p-4 rounded-2xl bg-card border border-border/50 hover:border-primary/50 hover:bg-primary/5 transition-all text-left group shadow-sm hover:shadow-lg hover:-translate-y-0.5"
                                                                                        >
                                                                                            <div className="p-3 rounded-xl bg-primary/10 text-primary group-hover:bg-primary group-hover:text-white transition-all shadow-inner">
                                                                                                <Play className="w-4 h-4" />
                                                                                            </div>
                                                                                            <div className="min-w-0 flex-1">
                                                                                                <p className="text-sm font-bold text-foreground truncate group-hover:text-primary transition-colors">
                                                                                                    {(() => {
                                                                                                        const name = v.file.replace(".mp4", "");
                                                                                                        const match = name.match(/^(\d+)[- ](.*)$/);
                                                                                                        if (match) {
                                                                                                            const [, num, title] = match;
                                                                                                            return `Lesson ${num}: ${title.split(/[ -]/).map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ")}`;
                                                                                                        }
                                                                                                        return name.split(/[ -]/).map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ");
                                                                                                    })()}
                                                                                                </p>
                                                                                                <p className="text-[9px] font-black text-muted-foreground uppercase tracking-wider group-hover:text-muted-foreground/80">Available VOD</p>
                                                                                            </div>
                                                                                        </button>
                                                                                    ))}
                                                                                </div>
                                                    ...
                                                    <div className="text-center py-12 space-y-4 rounded-3xl bg-muted/20 border border-dashed border-border/50">
                                                        <div className="w-16 h-16 bg-muted/50 rounded-full flex items-center justify-center mx-auto ring-1 ring-border/50">
                                                            <Video className="w-8 h-8 text-muted-foreground/30" />
                                                        </div>
                                                        <div>
                                                            <h4 className="font-bold text-lg">Coming Soon</h4>
                                                            <p className="text-muted-foreground text-sm">Class video recordings will appear here.</p>
                                                        </div>
                                                    </div>
                                                )}
                                            </div>
            ...
            {selectedVideo && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-xl animate-in fade-in duration-300">
                    <div className="w-full max-w-5xl px-4 flex flex-col items-center">
                        ...
                        <div className="w-full aspect-video bg-black rounded-3xl overflow-hidden shadow-2xl ring-1 ring-white/10 relative">
                            <video
                                src={`/classes/${selectedVideo.className}/${selectedVideo.module ? selectedVideo.module + '/' : ''}${selectedVideo.video}`}
                                controls
                                autoPlay
                        ...
                        <div className="mt-8 text-center space-y-2">
                            <p className="text-primary font-black uppercase tracking-[0.3em] text-[10px]">{selectedVideo.className} {selectedVideo.module && `• ${selectedVideo.module}`}</p>
                            <h2 className="text-white text-2xl font-black uppercase italic tracking-tighter">
                                {(() => {
                                    const name = selectedVideo.video.replace(".mp4", "");
                                    const match = name.match(/^(\d+)[- ](.*)$/);
                                    if (match) {
                                        const [, num, title] = match;
                                        return `Lesson ${num}: ${title.split(/[ -]/).map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ")}`;
